...
namespace Projeem.View
{
    public partial class frmAplicaDebitoECredito : Form
    {
        ...
        static decimal[] CtrlGpID = new decimal[37];
        static decimal[] CtrlGpDebito = new decimal[37];
        static decimal[] CtrlGpDebitoDeduzido = new decimal[37];
        static decimal[] CtrlGpCredito = new decimal[37];
            ...
            if (vTipoDeProcessamento.Equals("DEBITO"))
            {
                ...
                if (cboGrupoAtual.Text.Trim().Length <= 0) {
                    ...
                    try {                        
                        ...
                        /**/
                        //ProcessaCredito();
                        ProcessaCredito(Convert.ToInt32(cboGrupoAtual.Text));
                        /**/
                        Indices.UpdateStatusProcessamento("DEBITO", this.cn);
                        btnZeraBaseDeDados.Enabled = true; // Habilita após processamente das Séries
        ...
        #region:: Área de Inicialização do Método ProcessaCredito(int v)

        /// <summary>
        /// Método sobrecarregado, que vará uma varredura apenas
        /// nos registros, cujas séries contenha o valor sorteado!
        /// - Ou seja; ao invés de percorrer toda a tabela de serie
        /// nos concentraríamos apenas nas series premiadas, utilizando 
        /// uma consulta simples com a palavra chave "IN" na cláusula WHERE!
        /// </summary>
        /// <param name="vSorteado">Valor sorteado, escolhido na label de gr_premiado</param>
        private void ProcessaCredito(int vSorteado) {

            btnStartProcess.Enabled = false;
            SeriesPremiadas SRPremiadas = new SeriesPremiadas(this.cn);
            SRPremiadas.CarregaSeries(vSorteado);
            SRPremiadas.Primeiro();
            totalDeSerie = SRPremiadas.Num_registro;
            if (totalDeSerie > 0) {

                // Substitui o AumentaCarencia e o ZeraCarencia,
                // aumentando as carencias dos NÃO sorteados e 
                // zerandoas carencias dos sortedos!
                SRPremiadas.AumentaCarencia(vSorteado); // Depois altero para AtualizaCarencias
        ...
        #endregion: Fim do método sobrevcarregado ProcessaCredito Com parametros
        ...
        private void ProcessaCredito() 
        {
            ...
            while (!serie.FinalDeArquivo)
            {
                ...
                if ((gp_premiado == serie.Gr_01) || (gp_premiado == serie.Gr_02) || (gp_premiado == serie.Gr_03) || (gp_premiado == serie.Gr_04))
                {
                ...
                else
                {
                    ...
                    // "NÃO" for premiada; Crédito, Recap, Status, Subsídio, Etc.
        ...
        void InsereRegistro(int gr1, int gr2, int gr3, int gr4, int id)
        {
            ...
            cmd.Parameters.Add(new SqlParameter("@gr_04", gr4));
            cmd.Parameters.Add(new SqlParameter("@carencia", "0"));
            cmd.Parameters.Add(new SqlParameter("@qtde_premio", "0"));
